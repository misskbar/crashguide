...
class ThirdPartyPhotoFragment : BaseFragment(), View.OnClickListener {
    ...
    private val EMAIL_REQUEST_CODE = 103
    ...
    val catFont = Font(Font.FontFamily.TIMES_ROMAN, 24f, Font.BOLD);
...
//        setAll(photoPath)
    ...
    private fun sendEmail() {
        ...
        emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Informe de accidente Crash Guide");
...
//        var uri = Uri.fromFile(file);
        ...
        startActivityForResult(Intent.createChooser(emailIntent, "Pick an Email provider"),EMAIL_REQUEST_CODE);
    }
    ...
    private fun addTitlePage(document: Document) {
        ...
        val bitmap = BitmapFactory.decodeResource(resources, R.drawable.logo_grande)//Bitmap image gets the explosion image from the resources folder
        var baos = ByteArrayOutputStream();
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, baos);
        var image = Image.getInstance (baos.toByteArray())
        ...
        var para = Paragraph("Crash Guide", catFont)
        para.setAlignment(Image.MIDDLE )

        addEmptyLine(preface, 1)

        para = Paragraph("Informe de accidente", catFont)
        para.setAlignment(Image.MIDDLE )
        preface.add(para)
        ...
        preface.add(Paragraph(
                "Fecha: " + dateFormat.format(date), smallBold))
        addEmptyLine(preface, 1)
...
//        subCatPart.add(image)
        ...
        var  bm: Bitmap = BitmapFactory.decodeFile(usuario.fotoCarnet.toString())
        val height = 200
        val width = 300
        ...
        bm = BitmapFactory.decodeFile(usuario.fotoLicencia.toString())
        bm = Bitmap.createScaledBitmap(bm, width, height, true)
        baos = ByteArrayOutputStream();
        bm.compress(Bitmap.CompressFormat.JPEG, 100, baos); //bm is the bitmap object
        subCatPart.add(Paragraph("Foto de la licencia de conducir:"))
...
//        subCatPart.add(image)
        ...
        var  bm: Bitmap = BitmapFactory.decodeFile(imagePath.toString())
        val height = 200
        val width = 300
        bm = Bitmap.createScaledBitmap(bm, width, height, true)
        var baos = ByteArrayOutputStream();
